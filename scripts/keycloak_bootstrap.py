...
Description:
    ...
    backend/front-end clients, and initial users. This script uses the Keycloak
    Admin REST API to automate creation of realms and users, then prints the
    generated backend client secret for service-account usage.
...
Usage:
    python scripts/keycloak_bootstrap.py \
        ...
        --frontend-client-id backup-restore-frontend \
        --backend-client-id backup-restore-backend \
        --frontend-root-url http://localhost:8086 \
        --api-root-url http://localhost:8000 \
        --user admin:admin:backup:admin \
        --user operator:operator:backup:read,backup:create,backup:restore \
        --user viewer:viewer:backup:read

Granular Roles (Criticality Order):
    - backup:restore - HIGHEST: Restores backups, modifies production data
    - backup:delete  - HIGH: Deletes backup files permanently
    - backup:create  - MEDIUM: Creates new backups
    - backup:read    - LOW: View-only access to backups and stats
    - backup:admin   - FULL: All permissions including config changes
"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--realm", default="backup-restore", help="Realm name")
    parser.add_argument(
        "--frontend-client-id",
        default="backup-restore-frontend",
        help="Frontend client ID",
    )
    parser.add_argument(
        "--backend-client-id",
        default="backup-restore-backend",
