...
class Neo4jConfig(BaseModel):
    ...
    target_api_url: Optional[str] = Field(None, description="Optional: Target API URL to lock during restore (e.g., http://localhost:8000)")
    target_api_key: Optional[str] = Field(None, description="Optional: API key for target API lock endpoint")
